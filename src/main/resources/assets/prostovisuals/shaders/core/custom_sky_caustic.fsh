...
void main(){
 ...
 vec3 base=mix(vec3(.003,.002,.016),vec3(.025,.006,.060),clamp(rd.y*.55+.55,0.,1.));
 vec3 glow=mix(vec3(.27,.025,.78),vec3(.82,.07,.58),.5+.5*sin((p.x-p.y)*.23+t*.16));
 // Significantly toned down vs v3.2: still luminous, no retina burn.
 vec3 col=base+glow*c*(.42+.30*breath)*uAlpha+vec3(.08,.13,.55)*halo*.055*breath;
