...
public final class BlockOverlay extends Module {
    ...
    private void renderShape(EventRender3D.Game event, BlockPos pos, VoxelShape shape) {
        ...
        Color top = visualColor.resolve();
        Color bottom = new Color(
                Math.round(top.getRed() * 0.66f),
                Math.round(top.getGreen() * 0.66f),
                Math.round(top.getBlue() * 0.66f));
        ...
        try {
            // Tessellator uses a shared allocator. Finish the fill before the outline
            // builder is created, otherwise fill.end() can see an empty builder.
            if (fill.getValue() && fillA > 0.0f) {
                BufferBuilder fillBuffer = Tessellator.getInstance()
                        .begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_COLOR);
                appendShapeFill(fillBuffer, matrix, camera, shape, bx, by, bz,
                        topR, topG, topB, bottomR, bottomG, bottomB, fillA);
            ...
            if (lineA > 0.0f) {
                ...
                appendShapeOutline(lineBuffer, matrix, camera, shape, bx, by, bz,
                        topR, topG, topB, bottomR, bottomG, bottomB, lineA);
                var builtLines = lineBuffer.endNullable();
                if (builtLines != null) {
                    GL11.glLineWidth(Math.max(1.0f, Math.min(5.0f, lineWidth.getValue().floatValue())));
    ...
    private static void appendShapeFill(BufferBuilder buffer, Matrix4f matrix, Vec3d camera,
                                        VoxelShape shape, int bx, int by, int bz,
                                        float topR, float topG, float topB,
                                        float bottomR, float bottomG, float bottomB, float alpha) {
        if (shape.isEmpty()) {
            appendFillBox(buffer, matrix, camera,
                    bx - 0.001, by - 0.001, bz - 0.001,
                    bx + 1.001, by + 1.001, bz + 1.001,
                    topR, topG, topB, bottomR, bottomG, bottomB, alpha);
            return;
        }
    ...
    private static void appendShapeOutline(BufferBuilder buffer, Matrix4f matrix, Vec3d camera,
                                           VoxelShape shape, int bx, int by, int bz,
                                           float topR, float topG, float topB,
                                           float bottomR, float bottomG, float bottomB, float alpha) {
        if (shape.isEmpty()) {
            appendOutlineBox(buffer, matrix, camera,
                    bx - 0.001, by - 0.001, bz - 0.001,
                    bx + 1.001, by + 1.001, bz + 1.001,
                    topR, topG, topB, bottomR, bottomG, bottomB, alpha);
            return;
    ...
    private static void appendFillBox(BufferBuilder fillBuffer, Matrix4f matrix, Vec3d camera,
                                      double minX, double minY, double minZ,
                                      double maxX, double maxY, double maxZ,
                                      float topR, float topG, float topB,
                                      float bottomR, float bottomG, float bottomB, float fillA) {
    ...
    private static void appendOutlineBox(BufferBuilder lineBuffer, Matrix4f matrix, Vec3d camera,
                                         double minX, double minY, double minZ,
                                         double maxX, double maxY, double maxZ,
                                         float topR, float topG, float topB,
                                         float bottomR, float bottomG, float bottomB, float lineA) {
        float x0 = (float) (minX - camera.x);
        float y0 = (float) (minY - camera.y);
        float z0 = (float) (minZ - camera.z);
        float x1 = (float) (maxX - camera.x);
