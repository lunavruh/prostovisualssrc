...
import dev.prostovisuals.client.custommodels.CosmeticsManagerNative;
...
public final class CustomModels extends Module {
    ...
    private final BindSetting menuKey = new BindSetting("Customize Menu", new Bind(GLFW.GLFW_KEY_F9, false));
    private final StringSetting selectedModel = new StringSetting("Selected Model", "", () -> false, false);
    ...
    public CustomModels() {
        super("Customize", Category.Render, "3D models, heads, hats, weapons and pets");
    ...
    public void selectCosmetic(CosmeticEntry entry) {
        ...
        if (CosmeticsManagerNative.CHINA_HAT_PATH.equalsIgnoreCase(entry.relativePath())) {
            prostovisuals.getInstance().getCosmeticsManager().toggle(entry);
    ...
    public void selectDefaultPlayer() {
        FiguraCosmeticsEngine.clearLocal();
        if (CosmeticsManagerNative.CHINA_HAT_PATH.equalsIgnoreCase(
                prostovisuals.getInstance().getCosmeticsManager().getSelectedPath())) {
            prostovisuals.getInstance().getCosmeticsManager().clear();
        }
        selectedModel.setValue("");
        lastApplied = "";
        lastPlayerUuid = null;
    ...
    public boolean isSelected(CosmeticEntry entry) {
        ...
        if (CosmeticsManagerNative.CHINA_HAT_PATH.equalsIgnoreCase(entry.relativePath())) {
            return prostovisuals.getInstance().getCosmeticsManager().isSelected(entry);
        }
        return isFiguraSelected() && entry.relativePath().equalsIgnoreCase(getSelectedFiguraPath());
    }
