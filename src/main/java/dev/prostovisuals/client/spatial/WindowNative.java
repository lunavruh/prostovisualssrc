...
public final class WindowNative {
    ...
    /** Reject compositor placeholder frames (large flat gray/solid surfaces) while still allowing dark UIs. */
    public static boolean isDetailedFrameAbgr(int[] pixels, int width, int height) {
        if (!isUsefulFrameAbgr(pixels, width, height)) return false;
        int sx = Math.max(1, width / 32), sy = Math.max(1, height / 20);
        int samples = 0, varied = 0;
        int prev = -1;
        for (int y = sy / 2; y < height; y += sy) {
            int row = y * width;
            for (int x = sx / 2; x < width; x += sx) {
                ...
                if (prev >= 0 && Math.abs(lum - prev) >= 7) varied++;
                prev = lum;
                samples++;
            }
        }
        return samples > 0 && varied >= Math.max(4, samples / 90);
    }


    /**
     * Modern Chromium/Electron sometimes returns a correct title bar but a single flat
     * DirectComposition colour for the whole client area.  The old "detailed" test saw
     * the title bar variation and accepted that broken frame.  Judge the central content
     * separately so a gray browser body is rejected and a reliable fallback can be tried.
     */
    public static boolean isContentDetailedFrameAbgr(int[] pixels, int width, int height) {
        if (!isDetailedFrameAbgr(pixels, width, height)) return false;
        int x0 = Math.max(0, width / 10), x1 = Math.min(width, width - width / 10);
        int y0 = Math.max(0, height / 6), y1 = Math.min(height, height - height / 10);
        int sx = Math.max(1, (x1 - x0) / 24), sy = Math.max(1, (y1 - y0) / 14);
        int samples = 0, varied = 0, minL = 255, maxL = 0;
        int prev = -1;
        for (int y = y0 + sy / 2; y < y1; y += sy) {
            int row = y * width;
            for (int x = x0 + sx / 2; x < x1; x += sx) {
                int c = pixels[row + x];
                int r = c & 255, g = (c >>> 8) & 255, b = (c >>> 16) & 255;
                int lum = (r * 54 + g * 183 + b * 19) >>> 8;
                if (prev >= 0 && Math.abs(lum - prev) >= 6) varied++;
        ...
        if (samples == 0) return false;
        return (maxL - minL) >= 14 || varied >= Math.max(5, samples / 70);
    }
