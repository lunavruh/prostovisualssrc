...
public class ModuleComponent extends Component {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        Color themeAccent = ThemeManager.getInstance().getRenderedAccentColor();
        ...
        if (module.isToggled()) {
            Color accentOverlay = ThemeManager.getInstance().getRenderedAccentColor();
            Render2D.drawRoundedRect(context.getMatrices(), x + 1f, y + 1f, width - 2f, HEADER_HEIGHT - 2f, 7f,
        ...
        // Compact inline binds are hidden in the OneClient-style GUI; it has a dedicated Keybinds page.
        boolean showInlineBind = !(mc.currentScreen instanceof dev.prostovisuals.client.ui.clickgui.OneClientClickGui);
        ...
        if (showInlineBind) {
            ...
            Color bindAccent = ThemeManager.getInstance().getRenderedAccentColor();
            ...
            Render2D.drawFont(context.getMatrices(), Fonts.MEDIUM.getFont(bindButtonFont), bindButtonText,
                    bindTextX + (bindButtonW - Fonts.MEDIUM.getWidth(bindButtonText, bindButtonFont)) * 0.5f,
                    bindTextY + 2.6f, bindColor);
        } else {
            bindTextX = bindTextY = Float.NaN;
            bindTextW = bindTextH = 0f;
        }
    ...
    public void renderBindMenuOverlay(DrawContext context) {
        ...
        context.getMatrices().translate(0f, 0f, 600f);
            ...
            Color popupAccent = ThemeManager.getInstance().getRenderedAccentColor();
    ...
    public float renderSettingsExternally(DrawContext context, float contentX, float contentY, float contentW,
        ...
        boolean reference = mc.currentScreen instanceof dev.prostovisuals.client.ui.clickgui.OneClientClickGui;
        float rowHeight = reference ? 20f : CHILD_HEIGHT;
        float rowGap = reference ? 1.2f : 0f;
        context.enableScissor((int) clipX, (int) clipY, (int) (clipX + clipW), (int) (clipY + clipH));
        for (Component component : components) {
            if (!component.getVisible().get()) continue;
            component.setX(reference ? contentX + 1f : contentX + 5f);
            component.setY(childY);
            component.setWidth(reference ? contentW - 2f : contentW - 10f);
            component.setHeight(rowHeight);
            component.setGlobalAlpha(globalAlpha);
            component.render(context, mouseX, mouseY, delta);
            float h = component.getHeight() + component.getAddHeight().get();
            childY += h + rowGap;
            totalHeight += h + rowGap;
        }
        context.disableScissor();
        return totalHeight;
    }
