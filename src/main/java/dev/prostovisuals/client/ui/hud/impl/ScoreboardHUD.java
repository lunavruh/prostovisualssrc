    ...
    private void renderScoreboard(EventRender2D e) {
        ...
        try {
            ...
            Render2D.drawRoundedRect(e.getContext().getMatrices(), x + 1f, y + 1f,
                    width - 2f, height - 2f, 8f, new Color(8, 11, 17, 58));
            ...
            Render2D.drawRoundedRect(
                    ...
                    width - 10f, 1f, 0.5f,
                    new Color(accent.getRed(), accent.getGreen(), accent.getBlue(), 125)
            );
        } finally {
        ...
        e.getContext().getMatrices().translate(-x, -y, 0f);
        try {
            int titleX = Math.round(x + (width - mc.textRenderer.getWidth(title)) * 0.5f);
            int titleY = Math.round(y + top);
            e.getContext().drawTextWithShadow(
                    mc.textRenderer, title, titleX, titleY,
                    0xFFFFFFFF
            );
