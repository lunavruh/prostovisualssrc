...
public class TargetHud extends HudElement implements ThemeManager.ThemeChangeListener {
    ...
    private static final long TARGET_DURATION = 2500L;
    ...
    public void onRender2D(EventRender2D e) {
        ...
        // TargetHUD intentionally mirrors TargetEsp target acquisition/retention logic.
        ...
        // Same retention rules as TargetEsp: looking away does not immediately clear the target.
        // It stays for TARGET_DURATION, but dies immediately on death/invisibility/occlusion.
        if (lastTarget != null) {
            boolean killed = !lastTarget.isAlive();
            boolean expired = (now - lastSeenTime) > TARGET_DURATION;

            if (killed || expired) {
                lastTarget = null;
                forceFade = true;
            } else if (invisible(lastTarget)) {
                lastTarget = null;
                forceFade = true;
            } else {
                Vec3d eyes = mc.player.getCameraPosVec(e.getTickDelta());
        ...
        fadeAnimation.animate(show ? 1f : 0f, show ? 85 : 70);
        scaleAnimation.animate(show ? 1f : 0.992f, show ? 95 : 80);
        slideAnimation.animate(show ? 0f : 1f, show ? 105 : 90);
